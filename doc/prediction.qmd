...
All the levels of $\mathbf{K_a}$ that were included in the HMME
come with a Prediction Error Variance (PEV) computed as:

$$
\mathrm{PEV}(\hat{\mathbf{u}}) =
\mathbf{G}
...
in the sole inversion of the $\mathbf{K}_{train}$, the submatrix of $\mathbf{K}_a$
...
For `str`, $\mathbf{K}_a$ is read from the supplied covariance; for `ar`, it is
rebuilt from the supplied distance with the estimated decay,
$(\mathbf{K}_a)_{ij} = \exp(-\hat{\rho}\, D_{ij})$.
