...
plt.title("Genetic growth curves")
...
cov2cor(mod.random[1].variance["sigma"])
```
:::
...
as a factor (`C(year)`) in the heteroscedasticity specification. Also, permanent
environment (random regression with an `iid` right-hand term) is required to capture
individual non-genetic variation.
