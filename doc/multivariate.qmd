...
$\mathbf{u} \sim \mathcal{N}(\mathbf{0}, \bigoplus_\mathbf{a} \mathbf{\Sigma_a} \otimes
\mathbf{K_a})$, and the residual stacks likewise (see [training](training.qmd#reml)).
...
When assessing the same genotype in different environments (multi-environment trial), the residual correlation cannot be estimated, as no experimental unit is ever observed in several environments. Therefore, the residual left-hand factor should stay diagonal (`left_hand="diag"`):
...
is required as the specificities of the random effect
$\operatorname{diag}(\mathbf{\Psi_a})$ are close to zero in this example.
Also, it is a good practice to scale the different responses by their phenotypic
mean and variance so that their difference in magnitude is not driving the FA
decomposition.

The factor-analytic can be read as a principal component analysis. Let us
scale the genetic covariance to genetic correlation, interpret $\mathbf{\Lambda_a}$ as a raw
