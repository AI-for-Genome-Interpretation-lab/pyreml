...
[pyreml]{.pyreml} takes one or several columns as `response` and performs multivariate analysis by modeling the covariance between them [*e.g.* @calus_accuracy_2011; @jia_multiple-trait_2012]
...
=
\begin{bmatrix}\mathbf{X_1} & \mathbf{0}\\\mathbf{0} & \mathbf{X_2}\end{bmatrix}
\begin{bmatrix} \mathbf{\beta_{a1}} \\ \mathbf{\beta_{a2}} \end{bmatrix}
+
\begin{bmatrix}
\mathbf{Z_{a1}} & \mathbf{0} & \mathbf{Z_{b1}} & \mathbf{0}\\
\mathbf{0} & \mathbf{Z_{a2}} & \mathbf{0} & \mathbf{Z_{b2}}
\end{bmatrix}
\begin{bmatrix}
\mathbf{u_{a1}} \\ \mathbf{u_{a2}} \\ \mathbf{u_{b1}} \\ \mathbf{u_{b2}}
\end{bmatrix}
+
...
$$
\begin{bmatrix} \mathbf{u_{a1}} \\ \mathbf{u_{a2}} \end{bmatrix}
\sim
...
The full stacked random vector keeps the direct-sum form $\mathbf{u} \sim \mathcal{N}(\mathbf{0}, \bigoplus_\mathbf{a} \mathbf{\Sigma_a} \otimes \mathbf{K_a})$, and the residual stacks likewise (see [training](training.qmd#reml)).
This extends to any number of responses, random effects and random-regression components.
...
When assessing the same genotype in different environments (multi-environment trial), the residual correlation cannot be estimated, as no experimental unit is ever observed in several environments.
Therefore, the residual left-hand factor should stay diagonal (`left_hand="diag"`):

$$
\mathbf{R} =
\begin{bmatrix}\sigma^2_{R1} & 0 \\ 0 & \sigma^2_{R2}\end{bmatrix}
\otimes \mathbf{I}.
$$
...
When the number of responses grows, estimating a `full` $\mathbf{\Sigma_a}$ becomes expensive.
The [factor-analytic](variance_structures.qmd#factor-analytic) structure approximates it with a few axes, set by `n_axes`:
...
A [jitter](variance_structures.qmd#jitter) is required as the specificities of the random effect $\operatorname{diag}(\mathbf{\Psi_a})$ are close to zero in this example.
Also, it is a good practice to scale the different responses by their phenotypic mean and variance so that their difference in magnitude is not driving the FA decomposition.

The factor-analytic can be read as a principal component analysis.
Let us scale the genetic covariance to genetic correlation, interpret $\mathbf{\Lambda_a}$ as a raw amount of $\mathbf{\Sigma_a}$ explained by each axis, and study the structure of the genetic component of the traits in their correlative space.
...
This yields a correlation circle allowing direct interpretation of the factorial axes, and a dendrogram clustering the responses with regards to the random effect.
