...
the construction of the inverse of $\mathbf{K}_a$ can be broken down as a Kronecker
of inverses, but furthermore these elementary inverses have a close form:
...
$$
\log\lvert\mathbf{K}_k\rvert
  = (L_k-1)\log\bigl(1-\varphi_k^2\bigr).
$$

Given the dimensional rule, and that $\mathbf{K}_a$ is
built for the whole grid even if it is sparsed,
this optimizes the computation in the current dense implementation
