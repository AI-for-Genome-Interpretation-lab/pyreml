...
def prepare_pedigree(pedigree):

    MISSING_STRINGS = {"", "nan", "none", "na", "n/a", "null"}
    ...
    for col in ["id", "sire", "dam"]:
        if col in ped.columns:
            if pd.api.types.is_numeric_dtype(ped[col]):
                ped[col] = ped[col].apply(
                    lambda x: str(int(x)) if pd.notna(x) else np.nan
                )
            else:
                ped[col] = ped[col].apply(
                    lambda x: np.nan
                    if pd.isna(x) or str(x).strip().lower() in MISSING_STRINGS
                    else str(x).strip()
                )

    ped = ped.drop_duplicates(subset="id")
    all_parents = set(ped["dam"]).union(ped["sire"]) - {np.nan}
    founders = all_parents - set(ped["id"])
    founder_rows = pd.DataFrame({"id": list(founders), "dam": np.nan, "sire": np.nan})
    ped = pd.concat([founder_rows, ped], ignore_index=True)
    return ped
